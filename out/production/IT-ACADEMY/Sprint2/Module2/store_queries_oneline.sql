...
-- 5
select nombre, precio as 'euros', round((precio * 1.1) , 2) as dolares from producto;
...
-- 8
select nombre as 'nombre', upper(left(nombre, 2)) as 'initials' from fabricante;

-- 9
select nombre as 'nombre', round(precio) from producto;

-- 10
select nombre as 'nombre', truncate(precio, 0) from producto;
...
-- 23
select p.codigo as 'codigo_producto', p.nombre, f.codigo as 'codigo_fabricante', f.nombre from producto p join fabricante f on p.codigo_fabricante = f.codigo;

-- 24
select p.nombre, precio, f.nombre from producto p join fabricante f on p.codigo_fabricante = f.codigo where p.precio = (select min(precio) from producto);

-- 25
select p.nombre, precio, f.nombre from producto p join fabricante f on p.codigo_fabricante = f.codigo where p.precio = (select max(precio) from producto);
...
-- 35
select * from producto p join fabricante f on p.codigo_fabricante = f.codigo where p.nombre is null;

-- 36
select p.nombre from producto p left join fabricante f on p.codigo_fabricante = f.codigo where f.nombre = 'Lenovo';

-- 37
select * from producto p left join fabricante f on p.codigo_fabricante = f.codigo where precio = (select max(precio) from producto);
...
-- 41
select p.nombre from producto p join fabricante f on p.codigo_fabricante = f.codigo where f.nombre = 'Asus' AND precio >= (select avg(precio) from producto p join fabricante f on p.codigo_fabricante = f.codigo where f.nombre = 'Asus');